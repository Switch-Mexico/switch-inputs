...
@click.option("--ext", default=".csv", help="Extension for output files")
@click.pass_context
def create(ctx, number, existing, proposed, load, ext, path=default_path, **kwargs):
    ...
    if existing and proposed:
        ...
        gen_project_legacy = pd.read_csv(
            os.path.join(default_path, "generation_projects_info.tab"), sep="\t"
        )
        ...
        click.secho("Oops I do not know what to do yet. Blame the developer!", fg="red")
        sys.exit(1)

    # Finally
    gen_project.to_csv(
        os.path.join(output_path, "generation_projects_info.csv"), sep=",", index=False
    )
    ...
    for periods, row in periods_tab.iterrows():
        ...
        peak_data = get_peak_day(
            load_data.loc[str(periods), "total"], number, freq="1MS", **kwargs
        )
        median_data = get_median_day(
            load_data.loc[str(periods), "total"], number, freq="1MS", **kwargs
        )
        ...
        timeseries.append(create_strings(median_data, scale_to_period, identifier="M"))
