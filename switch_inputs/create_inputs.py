...
def get_load_data(
    path=default_path, filename="HighLoads.csv", total=False, *args, **kwargs
):
    ...
    try:
        ...
        raise FileNotFoundError(
            "File not found. Please verify the file is in: {}".format(
...
def get_peak_day(data, number, freq="MS"):
    ...
    for _, group in data.groupby([pd.Grouper(freq="A"), pd.Grouper(freq=freq)]):
...
def get_median_day(data, number, freq="MS"):
    ...
    for _, group in data.groupby([pd.Grouper(freq="A"), pd.Grouper(freq=freq)]):
        ...
        years.append(
            group.loc[index_median.strftime("%Y-%m-%d")]
            .iloc[:: int((24 / number))]
            .reset_index()
        )
    output_data = pd.concat(years, sort=True)
    output_data.rename(
        columns={"datetime": "date", "total": "median_day"}, inplace=True
    )
...
def create_rps(
    path=default_path,
    filename="rps_targets",
    output_name="rps_targets",
    ext=".yaml",
    output_ext=".csv",
):
...
def create_strings(data, scale_to_period, identifier="P", ext=".csv", **kwargs):
...
def create_timeseries(data, number, ext=".csv", **kwargs):
    ...
    timeseries = data[
        ["TIMESERIES", "daysinmonth", "ts_period", "scale_to_period"]
    ].drop_duplicates("TIMESERIES")
    timeseries = timeseries.reset_index(drop=True)

    timeseries["ts_duration_of_tp"] = 24 / number
    timeseries["no_timeseries"] = timeseries.groupby("ts_period")[
        "TIMESERIES"
    ].transform("nunique")
...
def create_variablecp(
    gen_project, data, timeseries_dict, path=default_path, ext=".csv", **kwargs
):
    ...
    ren_cap_data = pd.read_csv(
        os.path.join(path, file_name), index_col=0, parse_dates=True
    )
    ren_cap_data = ren_cap_data.loc[
        ren_cap_data["project_name"].isin(gen_project["GENERATION_PROJECT"])
    ]

    # Quick fix to names. I need to include more code here
    replaces = [("e", "é"), ("a", "á"), ("i", "í"), ("o", "ó"), ("u", "ú"), ("n", "ñ")]
    try:
        for tupl in replaces:
            ren_cap_data["GENERATION_PROJECT"] = ren_cap_data[
                "GENERATION_PROJECT"
            ].str.replace(tupl[1], tupl[0])
    except KeyError:
        pass

    # Extract datetime without year information
    filter_dates = pd.DatetimeIndex(data["date"].reset_index(drop=True)).strftime(
        "%m-%d %H:%M:%S"
    )
    ...
    for row, value in timeseries_dict.items():
        ...
        filter_dates = pd.DatetimeIndex(tmp2["date"].reset_index(drop=True)).strftime(
            "%m-%d %H:%M:%S"
        )
        grouped = (
            ren_tmp[ren_tmp["time"].isin(filter_dates)]
            .reset_index(drop=True)
            .groupby("project_name", as_index=False)
        )
        list1.append(
            pd.concat(
                [group.reset_index(drop=True) for name, group in grouped], sort=True
            )
        )
    ...
    for keys in variable_tab.groups.keys():
        ...
        data.rename(
            columns={
                "capacity_factor": "gen_max_capacity_factor",
...
def create_loads(load, data, ext=".csv", **kwargs):
    ...
    unstack_loads = (
        loads_tmp.loc[data["date"]]  # Get filter dates
    ...
    loads_tab = pd.concat(
        [group.reset_index() for _, group in unstack_loads.groupby(level=0)], sort=True
    )

    # Renaming columns
    loads_tab = loads_tab.rename(
        columns={"level_0": "LOAD_ZONE", 0: "zone_demand_mw", "level_1": "TIMEPOINT"}
    )
...
def create_gen_build_cost(
    gen_project, gen_legacy, ext=".csv", path=default_path, **kwargs
):
    ...
    df = gen_build_cost.sort_values("gen_fixed_om").drop_duplicates(
        subset=["GENERATION_PROJECT", "build_year"], keep="last"
    )
    ...
    for index in df.build_year.unique():
        ...
        for tech in df["gen_tech"].unique():
            if tech in cost_table["Technology"].unique():
                mask2 = (cost_table["Technology"] == tech) & (
                    cost_table["Year"] == index
                )
                df.loc[mask & (df["gen_tech"] == tech)]
                cost_table.loc[mask2, "gen_overnight_cost"].values[0]
                df.loc[
                    mask & (df["gen_tech"] == tech), "gen_overnight_cost"
                ] = cost_table.loc[mask2, "gen_overnight_cost"].values[0]
    return df
